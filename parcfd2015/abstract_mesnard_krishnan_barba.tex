...
\keywords{Immersed Boundary Method, PETSc, Parallel CFD application, Flying Snake}

\abstract{An open-source \texttt{PETSc}-based immersed boundary method code is implemented to solve the three-dimensional flow over a cylinder with accurate cross-sectional shape of the flying snake species \textit{Chrysopelea paradisi}. We use a projection approach of the immersed boundary method to analyse vorticity structures in the wake and explain the enhanced lift generation of the snake at a particular angle of attack.}
